...
if (file.exists(data_path)) {
  ...
  cat(glue("Loaded {nrow(df):,} observations with {ncol(df)} variables\n\n"))
...
cat(strrep("=", 60), "\n")
cat(glue("Total respondents: {nrow(df):,}\n\n"))

for (var in geo_vars) {
  if (var %in% names(df)) {
    n_units <- n_distinct(df[[var]], na.rm = TRUE)
    avg_per_unit <- nrow(df) / n_units
    cat(glue("{var}: {n_units:,} unique units (avg {round(avg_per_unit, 1)} respondents/unit)\n"))
  }
}
```
...
cat(glue("Analysis sample: {nrow(analysis_df):,} observations\n"))
cat(glue("Number of neighborhoods: {n_distinct(analysis_df$buurt_id):,}\n"))
