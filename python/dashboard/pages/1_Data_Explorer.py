...
with tab_dv:
    ...
    with col1:
        ...
        if figures.get('dv_distribution'):
            st.image(str(figures['dv_distribution']), caption="Distribution of DV_single (0-100 scale)")
        else:
            ...
            if 'DV_single' in filtered_df.columns:
                fig = create_distribution_histogram(
                    ...
                    xaxis_label="DV_single (0-100 scale)",
                    nbins=50
                )
    ...
    with col1:
        if 'b_perc_low40_hh' in filtered_df.columns:
            fig = create_distribution_histogram(
                ...
                title="Distribution of % Low-Income Households (Buurt level)",
                xaxis_label="b_perc_low40_hh (standardized)",
                nbins=40
            )
            st.plotly_chart(fig, use_container_width=True)
