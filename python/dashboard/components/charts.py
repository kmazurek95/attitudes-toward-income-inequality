...
def create_cluster_size_histogram(
    ...
    title: str = "Respondents per Neighborhood",
    height: int = 400
) -> go.Figure:
    """
    ...
    fig = px.histogram(
        ...
        nbins=30,
        title=title,
        labels={'value': 'Number of Respondents', 'count': 'Number of Neighborhoods'},
        color_discrete_sequence=[COLORS["primary"]]
    )
    ...
    fig.update_layout(
        xaxis_title="Cluster Size (respondents)",
...
def create_model_progression_chart(
    ...
    fig.update_layout(
        ...
        yaxis_title="Key Predictor Coefficient",
        height=height,
        legend=dict(
            orientation="h",
            yanchor="bottom",
